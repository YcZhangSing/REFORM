...
save_epoch_step = 1
need_wandb = False
consist_warm_step = 3e4
reason_answer_warmup=3e4
...
consist_warm_step_str = f"{consist_warm_step:.0e}".replace('+0', '')
reason_answer_warmup_str = f"{reason_answer_warmup:.0e}".replace('+0', '')
...
def train_model(rank, world_size, dataset_name, train_time, batch_size=6, use_lora=False, epochs=10, lr=1e-6, eval_steps=10, run_name=None, max_val_item_count=1000):
    ...
    for epoch in range(epochs):
        ...
        if rank == 0 and (epoch+1)%save_epoch_step==0:  # Only the main process saves the checkpoint
            ...
            save_model(model, processor, output_dir, optimizer=None, lr_scheduler=None, epoch=epoch)
